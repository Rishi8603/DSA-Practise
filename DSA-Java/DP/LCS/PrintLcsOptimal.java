...
public class PrintLcsOptimal {
    // Build the DP table where dp[i][j] stores the length of the LCS of s1[0...i-1] and s2[0...j-1].
    private static void tabulation(String s1, String s2, int[][] dp) {
        int n1 = s1.length();
        int n2 = s2.length();
        for (int i = 1; i <= n1; i++) {
            for (int j = 1; j <= n2; j++) {
    ...
    private static void backtrackDP(String s1, String s2, int i, int j, int[][] dp, Set<String> ans, String temp, Map<String, Boolean> mpp) {
        if (i == 0 || j == 0) {
